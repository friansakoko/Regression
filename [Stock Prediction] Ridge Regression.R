...
mse_ridge <- mean((y_test - test_data$Predicted_Ridge)^2)
print(paste("Mean Squared Error (Ridge):", round(mse_ridge, 4)))

# R-squared
ss_tot_ridge <- sum((y_test - mean(y_test))^2)
ss_res_ridge <- sum((y_test - test_data$Predicted_Ridge)^2)
r_squared_ridge <- 1 - (ss_res_ridge / ss_tot_ridge)
print(paste("R-squared (Ridge):", round(r_squared_ridge, 4)))
...
ggplot(test_data, aes(x = Date)) +
  ...
  geom_line(aes(y = Predicted_Ridge, color = "Predicted"), size = 1) +
  labs(title = "Ridge Regression: Actual vs Predicted Adjusted Close",
...
# Make predictions using the existing Ridge Regression model
new_data$Predicted <- predict(ridge_model, s = optimal_lambda, newx = x_new)
